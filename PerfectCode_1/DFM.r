...
data1 <- read.csv("C:/Users/Administrator/desktop/codeblocks2/Processed_datasets/Datasets_WithSC.csv",header = T,check.names = F)
...
#feature_index <- read.csv('C:/Users/Administrator/desktop/codeblocks2/Processed_datasets/feature_index.csv',fileEncoding = "gbk",header = F,stringsAsFactors = F)
#feature_index <- as.vector(unlist(feature_index[1]))
indexf <- subset(outt, t==(delta_t+1))
print(indexf)
#index_fitted <- subset(outt, t==(delta_t+1) & .rownames %in% feature_index)$.fitted
#print(index_fitted)
write.csv(indexf , 'C:/Users/Administrator/desktop/codeblocks2/Processed_datasets/Index_fitted.csv')
